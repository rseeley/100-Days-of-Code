...
urlpatterns = [
    ...
    url(r'^boards/(?P<pk>\d+)/topics/(?P<topic_pk>\d+)/posts/(?P<post_pk>\d+)'
        '/edit/$', views.PostUpdateView.as_view(), name='edit_post'),
    url(r'^new_post/$', views.PostCreateView.as_view(), name='new_post'),
    ...
    url(r'^reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-'
        '[0-9A-Za-z]{1,20})/$', auth_views.PasswordResetConfirmView.as_view(
